...
import { Loader2, ArrowLeft, Eye, EyeOff, GraduationCap, Camera, Upload } from "lucide-react";
    ...
    if (isLogin) {
      ...
      if (data.user) {
        setTimeout(async () => {
          ...
          await supabase.from("user_roles").update({ role: "lecturer" } as any).eq("user_id", data.user!.id);
        }, 1500);
      }
        ...
        {pendingApproval ? (
          <div className="bg-card rounded-xl border p-6 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Your staff account has been created. It requires administrator approval before you can sign in.
            </p>
            <button onClick={() => { setPendingApproval(false); setIsLogin(true); }} className="text-sm text-primary hover:underline mt-2">
              Back to Sign In
            </button>
          </div>
        ) : (
          <div className="bg-card rounded-xl border p-6">
            <div className="flex mb-6 bg-secondary rounded-lg p-1">
              <button onClick={() => setIsLogin(true)} className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${isLogin ? "bg-card shadow-sm" : "text-muted-foreground"}`}>
                Sign In
              </button>
              <button onClick={() => setIsLogin(false)} className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${!isLogin ? "bg-card shadow-sm" : "text-muted-foreground"}`}>
                Register
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {!isLogin && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground bg-secondary/50 p-2 rounded-lg">
                    📋 Your Staff ID will be generated automatically by the system after registration.
                  </p>
                </>
              )}
              <div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} className={`${inputClass} pr-10`} placeholder="••••••••" />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors">
                    {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
              </div>
              <button type="submit" disabled={loading} className="w-full py-2.5 rounded-lg bg-primary text-primary-foreground font-medium text-sm hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2">
